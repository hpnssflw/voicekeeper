...
export async function POST(request: Request) {
  try {
    ...
    const { ownerId, channelUsername, limit, offsetId, minDate } = body;
    ...
    const response = await fetch(`${BOT_API_BASE}/mtproto/channels/parse`, {
      method: 'POST',
      headers: {
    ...
    console.error('MTProto channel parse error:', error);
    return NextResponse.json(
      { error: error.message || 'Failed to parse channel. Make sure bot service is running.' },
      { status: 400 }
    );
  }
}
