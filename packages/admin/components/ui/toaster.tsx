  ...
  switch (action.type) {
    ...
    case "DISMISS_TOAST":
      if (action.toastId) {
        ...
        memoryToasts.forEach((t) => addToRemoveQueue(t.id));
      }
      break;
    case "REMOVE_TOAST":
      memoryToasts = action.toastId
        ? memoryToasts.filter((t) => t.id !== action.toastId)
        : [];
      break;
  }
...
function useToast() {
  ...
  return { toasts, toast, dismiss: (id?: string) => dispatch({ type: "DISMISS_TOAST", toastId: id }) };
}
